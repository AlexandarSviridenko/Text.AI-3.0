...
int editingUserText(string fragment)
{
	char word[N];
	int iw = 0;
	int counter = 0;
	...
	{
		if (!isSeparator(fragment[i]) and  !isNumber(fragment[i]))
		{
			word[iw++] = fragment[i];			
			if (isSeparator(fragment[i + 1]) or fragment[i + 1] == '\0' or isNumber(fragment[i+1])) 
			{
				word[iw] = '\0';
				iw = 0;
				for(int k = 0; word[k] != '\0'; k++){
